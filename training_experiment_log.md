...
## 2026-07-31 Test 人工审查与可信子集

### 输入

```text
人工审查表: test_manual_review_sheet.csv
原始 test: data/20260204111923/test
```
...
worst cases 中标签问题和建议剔除样本占比很高，说明原始 test Dice 会被 GT 噪声显著拉低。
因此后续报告不应只给原始 test 分数，还应同时报告 curated test 分数。
...
```text
data/20260204111923_curated_manual_review_20260731/test/images
data/20260204111923_curated_manual_review_20260731/test/masks
data/20260204111923_curated_manual_review_20260731/test/labels
```
...
```text
1. 任一记录 suggest_action 为 exclude_eval，排除。
2. 任一记录 suggest_action 为 fix_label / GT漏标 / GT错标 / GT漏标错标，排除。
3. 任一记录 gt_reliable = N，排除。
4. 未进入 worst-case 人工表的 test 图片暂时保留，标记为 unreviewed_not_worst。
5. 人工审查后 GT 可靠且无标签修正建议的 worst-case 图片保留，标记为 reviewed_trusted。
```
...
```text
docs/archive/2026-07-31/test_manual_review/
```
...
README.md
summary.json
test_manual_review_sheet.csv
curated_manifest.csv
excluded_images.csv
label_problem_images.csv
reviewed_trusted_images.csv
```
...
original A test
curated A test
curated A test ignore class2
```

curated test 只用于最终评估解释，不用于训练和调参。

## 2026-07-31 W 系列 Class2 权重消融

### 输入日志

```text
C:\Users\cjy\Downloads\logs\run_W_A_*.log
E:\project\ORO\runsTemp\runsABCtest\logs\W_A_*
```

### 实验目的

在固定 A 数据集上，测试 class2/inflammation 损失权重降低是否能减少噪声标签干扰。
主变量为 class2 权重，少量测试 class2 轻度过采样。

固定条件：

```text
dataset: data/20260204111923 fixed train/valid/test
model: smp_unet + resnet34 + imagenet
scale-factor: 0.75
loss: combined BCE/Dice = 0.7/0.3
augmentation: mild
val/test TTA: none
test threshold: 0.5
```
...
```text
run                                      best_val  epoch  test_dice  test_ignore_cls2  class0  class1  class2  class3
W_A_scale075_cls2w10_anchor              0.5665     21     0.5545       0.6787        0.6852  0.7894  0.4379  0.4898
W_A_scale075_cls2w05_os15                0.5646     18     0.5459       0.6786        0.6130  0.7860  0.3191  0.5589
W_A_scale075_cls2w075                    0.5627     57     0.5430       0.6908        0.6372  0.7506  0.5615  0.7344
U_A_20260204_anchor_scale075_cls2w10     0.5665     26     0.5430       0.6945        0.6124  0.7831  0.4041  0.7665
W_A_scale075_cls2w05_seed43              0.5581     23     0.5424       0.6832        0.6170  0.7835  0.4731  0.6561
W_A_scale075_cls2w05                     0.5617     26     0.5408       0.6920        0.6465  0.7951  0.3983  0.6461
W_A_scale075_cls2w075_os15               0.5679     28     0.5386       0.6917        0.5970  0.7884  0.3611  0.6452
W_A_scale075_cls2w075_seed43             0.5594     21     0.5363       0.6610        0.5777  0.7738  0.5239  0.6489
W_A_scale075_cls2w025                    0.5445     39     0.5236       0.6830        0.6470  0.7773  0.3371  0.6872
```

### 结论

```text
1. class2 降权没有带来整体 Dice 提升。
2. 当前最高 original A test 是 W_A_scale075_cls2w10_anchor，test_dice = 0.5545。
3. class2w075 能把 class2 Dice 拉到 0.5615，同时 class3 也较好，但 class0/class1 下降，总分没有超过 anchor。
4. class2w025 明显过度降权，test_dice 跌到 0.5236，不建议继续。
5. class2 过采样 os1.5 没有改善，反而降低 class2 Dice，暂不继续加大过采样。
6. seed43 复验没有出现反超，说明 class2w05/075 的收益不稳定。
```

### 下一步

```text
P0: 重新评估历史关键模型在 curated A test 上的表现。
P1: original A test 与 curated A test 同时报告，不再只看原始脏 test。
P2: 暂停 class2 降权和过采样路线。
P3: 优先做评估口径修正：dataset-global Dice / per-class global Dice，避免 batch-size 影响分数。
P4: 之后再尝试模型层面改进：EMA、top-k checkpoint averaging、decoder dropout、threshold calibration。
```

## 2026-07-31 旧人工审查表 Train/Valid 筛选

### 输入

```text
旧人工审查表:
docs/archive/2026-07-31/legacy_manual_review_input/manual_review_sheet.csv.xlsx

提取 CSV:
docs/archive/2026-07-31/legacy_manual_review_input/manual_review_sheet_extracted.csv

原始 A 数据:
data/20260204111923
```

该表来自 2026-07-24 左右的 manual review package，主要覆盖 TTA worst、TTA harmful、class2 hard 样本。
它不是当前 test 人工表；匹配当前 A 数据后，主要落在 train/valid：
...
只对 `train/valid` 应用筛选，`test` 保持原样复制以保证目录完整。
...
已生成 train/valid 清洗版 A 数据：

```text
data/20260204111923_trainval_review_filtered_20260731
```

数量：

```text
train images/masks/labels: 2748
valid images/masks/labels: 788
test images/masks/labels: 400
...
```text
docs/archive/2026-07-31/legacy_manual_review_filter
```
...
summary.json
filter_manifest.csv
excluded_trainval_images.csv
reviewed_kept_trainval_images.csv
manual_review_sheet_extracted.csv
```
...
```text
原始 A train/valid -> original/curated test
filtered A train/valid -> original/curated test
```

若 filtered 版本 val 更稳定且 curated test 不下降，说明清洗 train/valid 有收益。
若 original test 下降但 curated test 上升，说明之前原始 test 中的标签噪声确实干扰了判断。

---

## 2026-07-31：Y 系列数据清洗受控对照与人工审查计划

### 训练问题

当前最需要回答的问题不是继续增加网络复杂度，而是：

```text
移除人工确认的 40 个 train/valid 标签问题样本，是否能稳定改善 A 域泛化？
```

因此下一轮采用两个 seed 的成对对照：

```text
Y_A_original_scale075_seed42
Y_A_filtered_scale075_seed42
Y_A_original_scale075_seed44
Y_A_filtered_scale075_seed44
```

四条训练除 train/valid 数据版本外保持一致，全部使用原始 A test；训练后统一补充：

```text
A original test
A curated test
B_clean cross-source test（仅作参考）
```

`temp.sh` 已加入失败后继续运行和 `Y_*` checkpoint 过滤，避免一条失败终止整批任务，也避免重复评估全部历史模型。

### 可选模型小实验

默认不运行。若四条受控训练已经完成，只运行 filtered 数据上的 dropout 和 plateau 两条单变量测试：
...
```text
docs/MANUAL_SCREENING_PLAN_20260731.md
docs/manual_review_sheet_template.csv
```

核心安排：

```text
A train：只审异常与长期 worst cases
A valid：审查 150-200 张高风险与分层随机样本，不缩成过小 valid
